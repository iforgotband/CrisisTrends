...
(function () {
    ...
    function doViz(words) {
        ...
        vis = svg
            .append('g')
            .attr('transform', 'translate(' + [cloudWidth >> 1,
                    cloudHeight >> 1
                ] +
                ')');
        ...
        const maxscale = d3.scale.linear()
            .range([50, 120])
            .domain([10, 960])(cloudWidth);
    ...
    function draw(words, bounds) {
        const scale = bounds ? Math.min(
                ...
                cloudHeight / Math.abs(bounds[0].y - cloudHeight / 2)) /
            2 : 1;
        ...
        vis.attr('transform', 'translate(' + [cloudWidth >> 1,
            cloudHeight >> 1
        ] + ')scale(' + scale + ')');
    }

    document.getElementById('wordcloud-select')
        .addEventListener('change', function () {
            ...
            d3.json('data/words/' + issue + '.json', function (error,
                json) {
                if (error) {
                ...
                if (quotes !== undefined) {
                    quoteDiv.innerHTML = '<p>' + quotes[issue] +
                        '</p>';
                }
